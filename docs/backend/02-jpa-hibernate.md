...
**Tất cả timestamp HỆ THỐNG (createdAt, updatedAt, expiredAt):** dùng `Instant` UTC.
...
DB lưu UTC. Khi serialize JSON, Jackson trả ISO-8601 UTC:
...
FE convert sang Asia/Ho_Chi_Minh hiển thị `10:00 31/05/2026`.

**Giờ chiếu phim (showtime.startAt):** vì rạp ở Việt Nam → dùng `LocalDateTime` (giờ địa phương rạp). Không cần timezone vì cùng 1 rạp = cùng 1 múi giờ.
...
**Ngày sinh user:** `LocalDate` — chỉ ngày, không giờ, không zone.
