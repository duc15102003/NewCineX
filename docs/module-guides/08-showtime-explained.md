...
## 15. Bổ sung — Form giá vé ĐỘNG theo loại ghế của phòng

### Bài toán

Showtime entity hỗ trợ 5 loại giá: `basePrice` (STANDARD + HANDICAP), `vipPrice`, `couplePrice`, `sweetboxPrice` (nullable, fallback `couple × 2`), `deluxePrice` (nullable, fallback `vip × 1.5`). Tuy nhiên form admin tạo/sửa suất chiếu trước đây chỉ hiển thị 3 input (base/vip/couple) — admin không control được giá Sweetbox/Deluxe, và phòng IMAX (chỉ có STANDARD/VIP) vẫn bắt nhập `couplePrice` vô nghĩa.

### Giải pháp — pattern chuẩn industry (CGV/Lotte)

**Chọn phòng → query loại ghế có trong phòng → render input giá ĐỘNG.**

### Backend — endpoint mới

```
GET /api/rooms/{roomId}/seats/types
```
...
```json
{
  "data": {
    ...
    "roomName": "Cinema 3",
    "seatTypes": [
      { "seatType": "STANDARD", "count": 80 },
      { "seatType": "VIP", "count": 24 },
      { "seatType": "COUPLE", "count": 8 },
      { "seatType": "HANDICAP", "count": 2 }
    ]
  }
}
```
...
**Implementation:**
- `SeatRepository.countSeatsByTypeInRoom(roomId)` — JPQL `GROUP BY s.seatType` + filter `aisle = false` (lối đi không phải ghế thật).
- `SeatService.getSeatTypeSummary(roomId)` — gọi repo, map sang `RoomSeatTypeSummaryResponse`.
- `SeatController.getSeatTypeSummary` — endpoint `GET /api/rooms/{roomId}/seats/types`.

### Frontend — form động

**Hook**: `useRoomSeatTypes(roomId)` trong `useAdminRooms.ts` — enabled khi `roomId > 0`, staleTime 5 phút.

**Form refactor** (`ShowtimeFormDialog.tsx`):
1. Watch `roomId` → fetch seatTypes.
2. Render input dựa trên config `PRICE_TIERS`:
   - `basePrice` ← STANDARD hoặc HANDICAP (gộp 1 tier, NĐ 28/2012 không thu phụ phí ghế khuyết tật).
   - `vipPrice` ← VIP
   - `couplePrice` ← COUPLE
   - `sweetboxPrice` ← SWEETBOX
   - `deluxePrice` ← DELUXE
3. Tier nào phòng KHÔNG có → ẩn input.
4. Hiển thị số lượng ghế bên cạnh label (VD: "Giá VIP (đ) · 24 ghế").
5. Submit: chỉ gửi field cho loại ghế phòng có → BE auto-fill default cho field còn lại (theo RoomType từ SystemConfig).

### Validate tier price

Quy tắc thứ tự giá (chỉ check tier phòng có thật):
- `base ≤ vip ≤ couple ≤ sweetbox`
- `base ≤ vip ≤ deluxe`
- Sweetbox vs Deluxe độc lập (Sweetbox là couple cao cấp 2 ghế, Deluxe là single recliner).

### Quy tắc bất biến

- **Không thay đổi `BookingService.getPriceForSeat`** — logic fallback (sweetbox = couple × 2, deluxe = vip × 1.5) vẫn giữ. Showtime cũ không có sweetbox/deluxe price vẫn book được.
- **HANDICAP dùng chung `basePrice`** với STANDARD — phù hợp NĐ 28/2012 không thu phụ phí ghế khuyết tật.
- **Aisle (lối đi)** bị loại khỏi count vì không phải ghế thật.

## 16. Cảnh báo về mất dấu tiếng Việt trong file gốc
