...
export default function PaymentResultPage() {
  ...
  if (!isSuccess) {
    return (
      <div className="min-h-screen bg-[#051424] flex items-center justify-center text-white px-4">
    ...
    <div className="min-h-screen bg-[#051424] text-white py-10 px-4">
      <div className="max-w-lg mx-auto">
        ...
        <div className="text-center mb-8">
          ...
          {transactionCode && (
            <p className="text-xs text-gray-500 mt-2">Mã giao dịch: <span className="font-mono text-[#eab308]">{transactionCode}</span></p>
          )}
        </div>

        {/* Nếu API lỗi (chưa login / token hết hạn) → hiện thông báo + link */}
        {isError && (
          <div className="bg-[#0a1929] border border-white/5 rounded-xl p-6 text-center mb-6">
            <p className="text-gray-300 mb-4">Thanh toán đã được xử lý. Đăng nhập để xem chi tiết vé.</p>
            <div className="flex gap-3 justify-center">
              <Link to="/login"><Button className="bg-[#eab308] hover:bg-[#ca8a04] text-black font-semibold">Đăng nhập</Button></Link>
        ...
        {ticket ? (
          <Card className="bg-[#0a1929] border-white/5 text-white overflow-hidden">
            ...
            <div className="flex gap-4 p-5 border-b border-white/5">
              ...
              <div className="space-y-1 flex-1">
                <h2 className="font-bold text-lg text-[#eab308]">{ticket.movieTitle}</h2>
            ...
            <CardContent className="p-5 space-y-4">
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="font-mono text-[#eab308] font-bold tracking-wider">
                  {ticket.bookingCode}
                </span>
              </div>
              ...
              <div className="flex justify-between font-semibold border-t border-white/10 pt-3">
                <span>Tổng tiền</span>
                <span className="text-[#eab308] text-lg">{formatPrice(ticket.totalAmount)}</span>
              </div>
          ...
          <Card className="bg-[#0a1929] border-white/5 text-white p-6 text-center">
            <p className="text-gray-300">
              Đơn đặt vé đã được ghi nhận.
              {transactionCode && (
                <span className="block mt-1 text-sm text-gray-400">
                  Mã giao dịch: <span className="font-mono text-[#eab308]">{transactionCode}</span>
                </span>
              )}
            </p>
          </Card>
        ...
        <div className="mt-6 flex flex-col sm:flex-row gap-3 justify-center">
          <Link to="/my-tickets">
            <Button className="w-full sm:w-auto bg-[#eab308] hover:bg-[#ca8a04] text-black font-semibold">
              Xem vé của tôi
            </Button>
          </Link>
          <Link to="/movies">
