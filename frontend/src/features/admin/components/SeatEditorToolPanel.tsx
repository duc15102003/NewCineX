...
export default function SeatEditorToolPanel(props: SeatEditorToolPanelProps) {
  return (
    <div className="w-64 shrink-0 space-y-4">
      <ToolPicker types={props.types} activeTool={props.activeTool} onSelectTool={props.onSelectTool} />
      <Instructions />
      <Stats
        types={props.types}
        rows={props.rows}
  ...
  return (
    <div className="bg-[#201b11] border border-white/5 rounded-2xl p-4">
      ...
      <div className="space-y-1.5 mb-4">
        {seatTypes.map((t) => (
          <ToolButton key={t.key} meta={t} active={activeTool === t.key} onClick={() => onSelectTool(t.key)} />
        ))}
      </div>
...
function ToolButton({ meta, active, onClick }: { meta: SeatTypeMeta; active: boolean; onClick: () => void }) {
